                        ...
                        <div class="d-flex align-items-center justify-content-end my-3">
                            <div id="bulk-select-replace-element">
                                <a href="{{route('user.create')}}">
                                    <button class="btn btn-falcon-success btn-sm" type="button"><span
                        ...
                        <tbody id="bulk-select-body">
                            ...
                            <tr id="{{$user->id}}">
                                ...
                                <td class="align-middle">
                                    ...
                                    @endforeach
                                    @else
                                    <a href="{{ route('department.index') }}" data-bs-toggle="tooltip"
                                        data-bs-placement="top" title="Goto and assigned departments">
                                        <span class="badge bg-danger fs--2">No Department</span>
                                    </a>
                                    @endif
                                </td>
                                <td class="align-middle text-capitalize">{{optional($user->roles[0])->name}}</td>
                                <td class="align-middle text-end pe-3 text-capitalize" data-toggle="tooltip"
                                    data-placement="top" title="Change Status">
                                    @if($user->status == 'active')
                                    <span
                                        class="userID-{{$user->id}} badge bg-primary changeStatus">{{$user->status}}</span>
                                    @else
                                    <span
                                        class="userID-{{$user->id}} badge bg-danger changeStatus">{{$user->status}}</span>
                                    @endif
                                </td>
                                <td>
                                    <div class="btn-group btn-group-sm btn-group-flush" role="group"
                                        ...
                                        <a href="{{route('user.view',$user->id)}}">
                                            <button class="btn btn-falcon-info btn-sm" type="button"
